...
/**
 * YoutubeService is a service class that interacts with the YouTube API.
 * It provides methods to fetch playlists and videos from a specific YouTube channel.
 * The class uses the Google API Client Library for Java to interact with the YouTube API.
 * It uses OAuth 2.0 for authorization, which involves obtaining an access token from the Google Authorization Server and then using that token to authorize requests to the YouTube API.
 */
@Slf4j
@Service
public class YoutubeService {
  ...
  private GenericJson getMyPlaylists(YouTube youtubeService) throws IOException {
    YouTube.Playlists.List request = youtubeService.playlists().list("id,snippet,contentDetails,status")
        .setPrettyPrint(true)
  ...
  private GenericJson getPlaylistItems(YouTube youtubeService, String playlistId) throws IOException {
    YouTube.PlaylistItems.List request = youtubeService.playlistItems().list("id,snippet,contentDetails,status")
        .setMaxResults(50L).setPrettyPrint(true).setPlaylistId(playlistId);
  ...
  private GoogleAuthorizationCodeFlow getGoogleAuthorizationCodeFlow(NetHttpTransport httpTransport,
  ...
  private Credential getCredentialsAuthorizationCodeInstalledApp(GoogleAuthorizationCodeFlow flow) throws IOException {
    AuthorizationCodeInstalledApp authorizationCodeInstalledApp = new AuthorizationCodeInstalledApp(flow,
        new LocalServerReceiver(), new AuthorizationCodeInstalledApp.DefaultBrowser());
